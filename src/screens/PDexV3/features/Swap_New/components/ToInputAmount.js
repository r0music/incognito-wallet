...
const ToInputAmount = (props) => {
  const dispatch = useDispatch();
  const navigation = useNavigation();
  const swap = useSelector(swapSelector);

  const { tokenName = 'Incognito', hasMaxValue = false } = props;
  ...
  return (
    <View style={styled.container}>
      <TextInput
        ...
        onChangeText={() => {}}
      />
      {hasMaxValue && <MaxButton />}
    </View>
  );
};

ToInputAmount.defaultProps = {
  tokenName: PropTypes.string.isRequired,
  hasMaxValue: PropTypes.bool,
};

ToInputAmount.propTypes = {
  tokenName: '',
  hasMaxValue: false,
};

export default ToInputAmount;
