...
import {
  getLastUpdateFirmwareTime,
  setLastUpdatefirmwareTime,
...
export default class NodeService {
  ...
  static send = (
    ...
    return new Promise((resolve, reject) => {
      ...
      if (productId) {
        ...
        const mailProductId = `${productId}${MAIL_UID_FORMAT}`;
        const action = NodeService.buildAction(
          product,
          actionExcute,
          dataToSend,
          chain,
  ...
  static reset = async (device: Device, chain = 'incognito') => {
    try {
      if (!_.isEmpty(device)) {
        ...
        const dataResult = await Util.excuteWithTimeout(
          NodeService.send(
            device.data,
            actionReset,
            chain,
            Action.TYPE.PRODUCT_CONTROL,
          ),
          timeout,
        );
  ...
  static updateFirmware = async (device: Device, latestVersion) => {
    const lastUpdateFirmWare = await getLastUpdateFirmwareTime(
      device.QRCode,
      latestVersion,
    );
  ...
  static sendValidatorKey = async (
    ...
    if (!_.isEmpty(device) && !_.isEmpty(validatorKey)) {
      ...
      await NodeService.send(
        device.data,
        LIST_ACTION.START,
        chain,
        Action.TYPE.INCOGNITO,
        {
          ...params,
      ...
      const dataResult = await NodeService.send(
        device.data,
        LIST_ACTION.GET_IP,
        chain,
        Action.TYPE.PRODUCT_CONTROL,
      ...
      if (status === 1) {
        const action: Action = NodeService.buildAction(
          device.data,
          LIST_ACTION.START,
          params,
          chain,
  ...
  static fetchAndSavingInfoNodeStake = async (device) => {
    try {
      ...
      const data = await APIService.fetchInfoNodeStake({
  ...
  static checkUpdatingVersion = async (device: Device) => {
    ...
    try {
      const { data, status = 0 } =
        (await APIService.getSystemPlatform().catch((e) =>
          new ExHandler(e).showWarningToast(),
        )) ?? {};
      console.log(TAG, 'checkUpdatingVersion begin ');
      if (!_.isEqual(status, 0)) {
        console.log(TAG, 'checkUpdatingVersion begin01 ');
        const { created_at, id, status = 0, version = '' } = data;
  ...
  static sendZMQ = async (params) => {
    if (!_.isEmpty(params)) {
      ...
      const result = await Util.excuteWithTimeout(
        ZMQService.sendData(JSON.stringify(params)),
        4,
      ).catch((e) =>
        console.log(TAG, ' connectZMQ sendZMQ ----- catch error = ', e),
      );
      return result;
    }
    return '';
  ...
  static isWithdrawable = async (device: Device) => {
    try {
      const response = await APIService.getRequestWithdraw(
        device.PaymentAddress,
      );
